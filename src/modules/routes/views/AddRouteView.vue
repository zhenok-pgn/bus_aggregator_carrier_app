<template>
  ...
  <v-stepper v-model="step">
      ...
      <v-row>
        <v-col cols="12">
          <v-stepper-header>
            ...
            <v-stepper-item :value="4" :complete="step > 4">Характеристики</v-stepper-item>
          </v-stepper-header>
        </v-col>
      </v-row>
    </v-container>
    ...
    <v-stepper-actions
      @click:next="nextStep"
      @click:prev="prevStep"
      :next-text="step != 4 ? 'Далее' : 'Сохранить'"
      :prev-text="'Назад'"
...
import type { Route, RouteSchedule, Tariff } from '@/interfaces/route'
...
export default defineComponent({
  ...
  methods: {
    ...
    nextStep() {
      ...
      else if (this.step == 4) this.saveRoute()
    },
    prevStep() {
